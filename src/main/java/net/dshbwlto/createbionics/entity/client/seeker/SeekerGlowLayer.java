...
public class SeekerGlowLayer extends RenderLayer<SeekerEntity, SeekerModel<SeekerEntity>> {
    ...
    public void render(PoseStack poseStack, MultiBufferSource buffer, int packedLight, SeekerEntity livingEntity, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        if (livingEntity.isFueled()) {
            ...
            VertexConsumer vertexConsumer = buffer.getBuffer(RenderType.entityTranslucentEmissive(
                    ResourceLocation.fromNamespaceAndPath(CreateBionics.MOD_ID, "textures/entity/seeker/seeker_glow" + i + ".png")));
