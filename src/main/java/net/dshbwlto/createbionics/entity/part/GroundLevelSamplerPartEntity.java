...
public class GroundLevelSamplerPartEntity extends RobotPartEntity{
     ...
     */
    public GroundLevelSamplerPartEntity(@NotNull MultiPartRobot parent, Entity connectedTo, float width, float height, double xOffset, double yOffset, double zOffset, Item pickResult) {
        super(parent, connectedTo, width, height, xOffset, yOffset, zOffset, pickResult, false);
    }

    public boolean isColliding() {
        BlockState blockState = level().getBlockState(this.blockPosition().below());
        return !(blockState.getBlock() instanceof AirBlock || !blockState.canOcclude());
    }
}
